  ...
  return (
    <MenubarContext.Provider value={contextValue}>
      <BaseMenubar
        data-slot="menubar"
        className={cn(
          "flex h-6 items-center gap-0.5 rounded-[var(--app-radius-pill)] border border-border/70 bg-primary-bg/65 px-0.5 py-0.5",
          className,
        )}
        {...props}
      />
...
function MenubarTrigger({ className, ...props }: ComponentProps<typeof Menu.Trigger>) {
  return (
    <Menu.Trigger
      ...
      openOnHover
      className={cn(
        "ui-font ui-text-sm flex h-5 select-none items-center rounded-[var(--app-radius-control-sm)] px-1.5 text-text-lighter outline-none transition-[transform,background-color,color] duration-[var(--app-duration-fast)] ease-[var(--app-ease-smooth)] hover:bg-hover/50 hover:text-text active:scale-[var(--app-press-scale)] focus:bg-hover/50 focus:text-text data-[popup-open]:bg-hover/80 data-[popup-open]:text-text",
        className,
      )}
      {...props}
    />
    ...
    <Menu.Portal>
      ...
      >
        <Menu.Popup
          ...
          className={cn(
            "z-[10031] w-max min-w-60 max-w-[min(480px,calc(100vw-16px))] rounded-[var(--app-radius-menu)] border border-border bg-secondary-bg/95 p-1 shadow-[var(--shadow-popover)] backdrop-blur-sm",
...
function MenubarItem({ className, shortcut, onClick, children, ...props }: MenubarItemProps) {
  return (
    <Menu.Item
      ...
      className={cn(
        "ui-font ui-text-sm flex min-h-7 cursor-default select-none items-center justify-between gap-6 rounded-[var(--app-radius-menu-item)] px-2.5 py-1.5 text-text outline-none transition-[background-color,color] duration-[var(--app-duration-fast)] ease-[var(--app-ease-smooth)] focus:bg-hover focus:text-text data-[highlighted]:bg-hover data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        className,
      )}
      {...props}
      onClick={(event) => {
...
function MenubarSubTrigger({
  ...
  return (
    <Menu.SubmenuTrigger
      ...
      openOnHover
      className={cn(
        "ui-font ui-text-sm flex min-h-7 cursor-default select-none items-center rounded-[var(--app-radius-menu-item)] px-2.5 py-1.5 text-text outline-none transition-[background-color,color] duration-[var(--app-duration-fast)] ease-[var(--app-ease-smooth)] focus:bg-hover focus:text-text data-[highlighted]:bg-hover data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        className,
      )}
      {...props}
    >
    ...
    <Menu.Portal>
      ...
      >
        <Menu.Popup
          ...
          className={cn(
            "z-[10050] w-max min-w-60 max-w-[min(480px,calc(100vw-16px))] rounded-[var(--app-radius-menu)] border border-border bg-secondary-bg/95 p-1 shadow-[var(--shadow-popover)] backdrop-blur-sm",
