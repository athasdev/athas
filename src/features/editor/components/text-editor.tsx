...
import { basicEditingExtension } from "@/features/editor/extensions/builtin/basic-editing";
import {
  setSyntaxHighlightingFilePath,
  syntaxHighlightingExtension,
} from "@/features/editor/extensions/builtin/syntax-highlighting";
...
import {
  ...
  useRAFCallback,
} from "@/features/editor/hooks/use-performance";
...
import { handleKeyboardShortcuts } from "../config/keyboard-bindings";
...
export function TextEditor() {
  ...
  const { setCursorVisibility, restorePositionForFile } = useEditorStateStore.use.actions();
  const activeBufferId = useBufferStore.use.activeBufferId();
  const { onChange, disabled, filePath, editorRef } = useEditorStateStore();
  const { setViewportHeight } = useEditorStateStore.use.actions();
  ...
  const { setCursorPosition, setSelection, setDesiredColumn } = useEditorStateStore.use.actions();
  const currentDesiredColumn = useEditorStateStore.use.desiredColumn?.() ?? undefined;
      ...
      try {
        ...
        if (!extensionManager.isExtensionLoaded("Auto Pairing")) {
          const { autoPairingExtension } = await import(
            "@/features/editor/extensions/builtin/auto-pairing"
          );
  ...
  const handleMoveLineUp = useCallback(() => {
    const cursorPos = useEditorStateStore.getState().cursorPosition;
