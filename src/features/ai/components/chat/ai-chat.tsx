...
const AIChat = memo(function AIChat({
	...
	const [surfaceStreamingMessageId, setSurfaceStreamingMessageId] = useState<
		string | null
	>(null);
	...
	const currentAgentId =
		currentChat?.agentId ?? useAIChatStore.getState().selectedAgentId;
	...
	const activeMessageSearchMatch =
		messageSearchMatches[activeMessageSearchIndex] ?? null;
	...
	const goToNextMessageSearchMatch = useCallback(() => {
		...
		setActiveMessageSearchIndex(
			(index) => (index + 1) % messageSearchMatches.length,
		);
	}, [messageSearchMatches.length]);
	...
	useEffect(() => {
		...
		setActiveMessageSearchIndex((index) =>
			Math.min(index, messageSearchMatches.length - 1),
		);
	}, [messageSearchMatches.length]);
			...
			unlisten = await listen<AcpEvent>("acp-event", ({ payload }) => {
				...
				switch (payload.type) {
					...
					case "session_info_update": {
						const chat =
							store.chats.find(
								(item) => item.acpSessionId === payload.sessionId,
							) ??
							(store.acpStatus?.sessionId === payload.sessionId
								? actions.getCurrentChat()
								: null);
						const nextTitle = chat
							? getChatTitleFromSessionInfo(chat.title, payload.title)
							: null;
						if (chat && nextTitle) {
	...
	const updateInitialAgentSessionTitle = useCallback(
		async (chatId: string, userMessage: string) => {
			...
			const managedPolicy = enterprisePolicy?.managedMode
				? enterprisePolicy
				: null;
	...
	const buildContext = async (
		...
		const selectedActiveBuffer =
			...
			selectedBufferIds.has(activeBuffer.id)
				? activeBuffer
				: undefined;

		let activeBufferContext:
			| (typeof activeBuffer & { webViewerContent?: string })
			| undefined = selectedActiveBuffer;
		if (
			selectedActiveBuffer?.type === "webViewer" &&
			selectedActiveBuffer.url
		) {
			const { fetchWebPageContent } = await import(
				"@/features/ai/services/web-content-service"
			);
		...
		if (selectedActiveBuffer && selectedActiveBuffer.type !== "webViewer") {
			const extension =
				selectedActiveBuffer.path.split(".").pop()?.toLowerCase() || "";
			...
			try {
				...
				await Promise.all(
					pendingPermissions.map((item) =>
						CodexIntegrationService.respond(item.requestId, false),
					),
				);
			} catch (error) {
	...
	const updateStreamingAssistantMessage = useCallback(
		(
			...
			const currentMessages = useAIChatStore
				.getState()
				.actions.getMessagesForChat(chatId);
	...
	const processMessage = async (
		...
		const targetChat = effectiveChatId
			...
			: null;
		const currentAgentId =
			targetChat?.agentId ?? store.actions.getCurrentAgentId();
		...
		if (!targetChatId) {
			...
			targetChatId = chatActions.ensureChatSession(
				targetChatId,
				currentAgentId,
				{
					activate: !chatId,
		...
		const existingMessages = useAIChatStore
			.getState()
			.actions.getMessagesForChat(targetChatId);
		const editedUserMessageIndex = options.editedUserMessageId
			? existingMessages.findIndex(
					(message) =>
						message.id === options.editedUserMessageId &&
						message.role === "user",
				)
			: -1;
			...
			editedUserMessageIndex >= 0
				? {
						...
						images:
							images.length > 0
								? images
								: existingMessages[editedUserMessageIndex].images,
					}
				: {
						id: createMessageId(),
						content: trimmedMessageContent,
		...
		try {
			...
			const attachedFiles = isAcp
				...
				: await loadFilesByPaths(
						Array.from(selectedFilesPaths).filter(
							(path) => !mentionedPaths.has(path),
						),
					);
					...
					if (directAction.kind === "open_web_viewer" && directAction.url) {
						if (!useSettingsStore.getState().settings.coreFeatures.webViewer) {
							chatActions.updateMessage(
								...
								{
									content:
										"Web Viewer is disabled. Enable it in Settings > Features to open URLs.",
									isStreaming: false,
								},
							);
							setIsSurfaceTyping(false);
						...
						directAction.kind === "open_terminal" &&
			...
			const enhancedMessage = isAcp
				? trimmedMessageContent
				: appendReferencedFiles(trimmedMessageContent, [
						...mentionedFiles,
						...attachedFiles,
					]);
			if (isAcp) {
				...
				() => {
					...
					if (!hasVisibleResponse) {
						if (isAcpAgent(currentAgentId) && acpProducedStateOnlyUpdate) {
							const slashCommand =
								trimmedMessageContent.match(/^\/([^\s]+)/)?.[1];
							const fallbackContent =
								acpCommandResultLabel ||
								(slashCommand
									? `Applied \`/${slashCommand}\`.`
									: "Session updated.");
						...
						const emptyResponseSource = isAcp
							? "agent session"
							: "provider request";
						updateStreamingAssistantMessage(
							targetChatId,
...
[/ERROR_BLOCK]`,
				...
				(error: string, canReconnect?: boolean) => {
					...
					const isAcpConfigError =
						...
						isAcpConfigurationError(mainError, errorDetails);
					...
					const shouldSuppressToast =
						isAcpAgent(currentAgentId) &&
						(mainError.includes("did not return any response") ||
							errorCode === "RECONNECT");
...
[/ERROR_BLOCK]`;
				...
				(event) => {
					appendAcpEvent({
						...
						detail:
							event.description ||
							`${event.permissionType} ${event.resource}`.trim(),
						state: "info",
					});
	...
	const isNewSession =
		(currentChat?.messages.length ?? 0) === 0 && acpEvents.length === 0;
	...
	const handlePermission = async (approved: boolean, optionId?: string) => {
		...
		try {
			const option = currentPermission.options.find(
			...
			if (currentAgentId === CODEX_INTEGRATION_ID) {
				await CodexIntegrationService.respond(
								...
								<MessageScrollerViewport>
									<ChatMessages
										...
										activeSearchMessageId={
											activeMessageSearchMatch?.messageId ?? null
										}
