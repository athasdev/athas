...
function MetadataRow({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="grid grid-cols-[1rem_4.25rem_minmax(0,1fr)] items-center gap-2">
      <span className="flex size-4 items-center justify-center text-text-lighter">{icon}</span>
      <dt className="text-text-lighter">{label}</dt>
        ...
        <HoverCardTrigger
          ...
          render={
            <button
              type="button"
              className={cn(
                "flex min-h-6 w-full min-w-0 items-center gap-2 rounded-md py-1 pr-12 pl-2 text-left text-text-lighter ui-text-sm transition-colors",
                "hover:bg-hover hover:text-text focus-visible:bg-hover focus-visible:text-text focus-visible:outline-none",
                active && "text-text",
              )}
            />
          }
      ...
      >
        <div className="border-border/70 border-b p-3">
          <div className="flex min-w-0 items-start gap-2.5">
            <span className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-primary-bg">
              <ProviderIcon providerId={providerIconId} size={16} />
            </span>
            <div className="min-w-0 flex-1">
              <div className="line-clamp-2 font-medium text-text ui-text-base">{title}</div>
              <div className="mt-1 text-text-lighter ui-text-sm">{formattedDate}</div>
            </div>
          </div>
        </div>
        ...
        {workspacePath ? (
          <div
            className="truncate border-border/70 border-t px-3 py-2 font-mono text-text-lighter ui-text-xs"
            title={workspacePath}
          >
            {workspacePath}
          </div>
