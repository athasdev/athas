...
import type {
	...
	PastedImage,
} from "@/features/ai/types/chat-composer.types";
...
const AIChatInputBar = memo(function AIChatInputBar({
	...
	const availableSlashCommands = useAIChatStore(
		(state) => state.availableSlashCommands,
	);
	...
	const showSlashCommands = useCallback(
		(position: InlineDropdownPosition, search: string) => {
	...
	useEffect(() => {
		const handleSidebarResourceDropOnAI = (event: Event) => {
			...
			const resource = (
				event as CustomEvent<{ resource?: SidebarDragResource }>
			).detail?.resource;
			if (!resource) return;
	...
	const getTextBeforeCaret = useCallback(
		() => getComposerTextBeforeCaret(inputRef.current),
		[],
	);
	...
	const selectedContextItems = useMemo(() => {
		const bufferSelections = buffers
			...
			.map((buffer) => ({
				...
				name: buffer.name,
				databaseType:
					buffer.type === "database" ? buffer.databaseType : undefined,
		...
		if (slashCommandState.active) {
			if (e.key === "ArrowDown") {
				...
				if (filteredSlashCommands[slashCommandState.selectedIndex]) {
					handleSlashCommandSelect(
						filteredSlashCommands[slashCommandState.selectedIndex],
					);
				}
			...
			if (e.key === "ArrowDown") {
				...
				setSelectedIndex(
					lastIndex < 0
						? 0
						: Math.min(mentionState.selectedIndex + 1, lastIndex),
				);
			} else if (e.key === "ArrowUp") {
				e.preventDefault();
			...
			if (selection && selection.rangeCount > 0 && inputRef.current) {
				...
				if (container === inputRef.current) {
					...
					const candidateNode =
						inputRef.current.childNodes[candidateIndex] ?? null;
				...
				if (
					...
					const previousSibling =
						container.previousSibling?.previousSibling ?? null; // Skip the space node
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener(AI_CHAT_INSERT_SKILL_EVENT, handleInsertSkill);
	...
	const handleSlashCommandSelect = useCallback(
		(command: SlashCommand) => {
			...
			hideSlashCommands();
			const commandRange = getComposerTextRange(
				inputRef.current,
				startIndex,
				endIndex,
			);
		...
		>
			<ChatComposerBody>
				{pastedImages.length > 0 && (
					<AttachmentGroup
						className={cn("px-3 pt-3", isInitialPresentation && "px-4 pt-4")}
					>
				...
				<ChatComposerEditable
					...
					className={cn(
						isInitialPresentation &&
							"max-h-48 min-h-28 overflow-y-auto px-4 py-4 ui-text-base",
					)}
					role="textbox"
				...
				<ChatComposerToolbar
					className={cn(isInitialPresentation && "items-center px-3 pb-3 pt-0")}
				>
							...
							<Attachment
								...
								onKeyDown={(event) => {
									...
									if (event.key === "Backspace" || event.key === "Delete") {
										...
										const chips = Array.from(
											chipContainer?.querySelectorAll<HTMLElement>(
												"[data-context-chip]",
											) || [],
										);
										const currentIndex = chips.indexOf(event.currentTarget);
										const nextFocusIndex = Math.max(
											0,
											Math.min(currentIndex, chips.length - 2),
										);
										if (item.type === "buffer") {
