...
describe("calculateDisplayNames", () => {
  it("normalizes Windows separators when distinguishing filenames", () => {
    const names = calculateDisplayNames([
      makeEditorBuffer("a", "C:\\workspace\\client\\index.ts"),
      makeEditorBuffer("b", "C:\\workspace\\server\\index.ts"),
    ]);
    expect(names.get("a")).toBe("../client/index.ts");
    expect(names.get("b")).toBe("../server/index.ts");
  });

  it("does not add separators to duplicate files at the filesystem root", () => {
    const names = calculateDisplayNames([
      makeEditorBuffer("a", "/index.ts"),
      makeEditorBuffer("b", "/index.ts"),
    ]);
    expect([...names.values()]).toEqual(["index.ts", "index.ts"]);
  });
  ...
  it("uses the buffer name for virtual buffers", () => {
    const names = calculateDisplayNames([
      makeEditorBuffer("term", "", true),
      makeEditorBuffer("file", "/workspace/a.ts"),
    ]);

    expect(names.get("term")).toBe("");
