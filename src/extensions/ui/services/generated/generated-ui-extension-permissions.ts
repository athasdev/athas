...
function parseNetworkOrigin(value: unknown, index: number): string {
  if (typeof value !== "string" || value.includes("*")) {
    throw new Error(
      `Generated integration network permission ${index + 1} must be an exact origin`,
    );
  }
  ...
  try {
    ...
    throw new Error(`Generated integration network permission ${index + 1} is invalid`);
  }
  ...
  if ((url.protocol !== "https:" && !localHttp) || url.username || url.password) {
    throw new Error(
      `Generated integration network permission ${index + 1} must use HTTPS or local HTTP`,
    );
  }
  if (url.pathname !== "/" || url.search || url.hash) {
    throw new Error(
      `Generated integration network permission ${index + 1} must not include a path`,
    );
  }
...
export function parseGeneratedExtensionPermissions(value: unknown): ExtensionPermissions {
  ...
  if (!isRecord(value)) throw new Error("Generated integration permissions must be an object");

  const unsupportedKey = Object.keys(value).find((key) => !PERMISSION_KEYS.has(key));
  if (unsupportedKey) {
    throw new Error(`Generated integration permission is not supported: ${unsupportedKey}`);
  }

  let network: string[] | undefined;
  if (value.network != null) {
    if (!Array.isArray(value.network) || value.network.length === 0) {
      throw new Error("Generated integration network permissions must be a non-empty array");
    }
    if (value.network.length > MAX_NETWORK_ORIGINS) {
      throw new Error(
        `Generated integrations may request at most ${MAX_NETWORK_ORIGINS} network origins`,
      );
    }
  ...
  if (value.secrets != null && typeof value.secrets !== "boolean") {
    throw new Error("Generated integration secrets permission must be a boolean");
  }
  if (value.workspace != null && value.workspace !== "read") {
    throw new Error('Generated integration workspace permission must be "read"');
  }
  if (value.openExternal != null && typeof value.openExternal !== "boolean") {
    throw new Error("Generated integration external-link permission must be a boolean");
  }
  if (value.clipboardWrite != null && typeof value.clipboardWrite !== "boolean") {
    throw new Error("Generated integration clipboard-write permission must be a boolean");
  }
...
export function validateGeneratedExtensionPermissionUsage(
  ...
  for (const capability of capabilities) {
    if (capability.used && !capability.granted) {
      throw new Error(
        `Generated integration uses ${capability.label} without requesting permission`,
      );
    }
    if (!capability.used && capability.granted) {
      throw new Error(`Generated integration requests unused ${capability.label} permission`);
    }
  }
}
