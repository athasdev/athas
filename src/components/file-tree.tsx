  ...
  const handleDrop = async (
    ...
    if (draggedItem) {
      ...
      if (!sourceParentPath || isRootLevelFile) {
        ...
        console.log(
          "6. Using root folder path as parent:",
          sourceParentPath || "(empty - will refresh root)",
        );
      }

      console.log(
        "=== FINAL: Source parent path for refresh:",
        sourceParentPath || "(root)",
        "===",
      );
      console.log(
        "This is the directory that will be refreshed to remove the moved file.",
      );
      ...
      try {
        ...
        if (onRefreshDirectory) {
          ...
          if (targetPath !== sourceParentPath) {
            console.log(
              `>>> Attempting to refresh target directory FIRST: "${targetPath}"`,
            );
            try {
              await onRefreshDirectory(targetPath);
              console.log(
                `✓ Target directory refresh completed: "${targetPath}"`,
              );
            } catch (error) {
              console.error(
                `✗ Failed to refresh target directory: "${targetPath}"`,
                error,
              );
            }
          }
          ...
          console.log(
            `>>> Attempting to refresh source directory: "${sourceParentPath || "(root)"}"`,
          );
          try {
            await onRefreshDirectory(sourceParentPath);
            console.log(
              `✓ Source directory refresh completed: "${sourceParentPath || "(root)"}"`,
            );
          } catch (error) {
            console.error(
              `✗ Failed to refresh source directory: "${sourceParentPath || "(root)"}"`,
              error,
            );
          }
          ...
          console.warn(
            "onRefreshDirectory is not defined, cannot refresh file tree",
          );
        }
      ...
      return (
        <div key={itemKey}>
          <button
            ...
            className={`w-full text-left px-1.5 py-1 bg-transparent border-none text-[var(--text-color)] text-xs font-mono flex items-center gap-1.5 transition-colors duration-150 whitespace-nowrap overflow-hidden text-ellipsis min-h-[22px] shadow-none outline-none hover:bg-[var(--hover-color)] focus:outline-none ${
