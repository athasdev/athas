...
/// only reapplies it on resize. Mirrors tao's `inset_traffic_lights`.
#[cfg(target_os = "macos")]
pub fn apply_traffic_light_position(ns_window: *mut c_void) {
   ...
   unsafe {
      ...
      // Only overlay title bars carry the custom inset; full screen hides the buttons anyway.
      ...
      let close: *mut Object = msg_send![window, standardWindowButton: 0u64];
      let miniaturize: *mut Object = msg_send![window, standardWindowButton: 1u64];
      let zoom: *mut Object = msg_send![window, standardWindowButton: 2u64];
      if close.is_null() || miniaturize.is_null() || zoom.is_null() {
         return;
      }
      let buttons_view: *mut Object = msg_send![close, superview];
      if buttons_view.is_null() {
         return;
      }
      let title_bar: *mut Object = msg_send![buttons_view, superview];
      ...
      let title_bar_rect: NSRect = msg_send![title_bar, frame];
      let height = close_rect.size.height + y;
      let next_title_bar = NSRect {
         origin: NSPoint {
            x: title_bar_rect.origin.x,
            y: window_rect.size.height - height,
         },
         size: NSSize {
            width: title_bar_rect.size.width,
            height,
         },
      };
      let _: () = msg_send![title_bar, setFrame: next_title_bar];

      let miniaturize_rect: NSRect = msg_send![miniaturize, frame];
      let spacing = miniaturize_rect.origin.x - close_rect.origin.x;
