...
fn project_config_path(workspace_path: &Path) -> PathBuf {
   workspace_path.join(".athas").join("docker.json")
}

fn read_project_config(workspace_path: &Path) -> Result<DockerProjectConfig, String> {
...
fn ensure_workspace_dir(workspace_path: &Path) -> Result<(), String> {
...
fn resolve_workspace_file(workspace_path: &Path, path: String) -> Result<PathBuf, String> {
   let requested_path =
