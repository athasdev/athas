...
- `vp check` runs `@shadcn/lint` and fails on unknown classes, undeclared colors, off-scale values, and overlay sizing set through `className`; its messages name the token or preset to use. `bun check:design` reports the wider policy that is not enforced yet (restyled primitives, inline styles, dynamic class names). It is advisory and never fails the build, but a change should not add findings to it.
